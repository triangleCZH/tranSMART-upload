...
//end of click function
    ...
    l3PluginPanel.render(document.body);
    ...
    resultsTabPanel.add(l3PluginPanel);
...
function updateL3Panel(html, insert) { //l3test probably this is about, drag -> analysis -> clear -> re drag
    if (insert) { //l3todo:does not involve in jQuery yet 
        //var div = jQuery("#analysisPanel div.analysis");
        //var uniq = 'appenedItem_' + new Date().getTime();
        //div.append(jQuery(html).attr('id', uniq));
        //div.parent().scrollTop(jQuery('#' + uniq).prop('offsetTop'));
    } else {
        l3PluginPanel.body.update(html, false, null); //l3test  panel.html=html doesn't work!
    }//l3todo: add html back!
}
...
function storeL3oaded(jsonStore, rows, paramsObject) {
	...
	if (fields[i].header.indexOf("<=age<") !== -1) { //detect age filter
	    ...
	    var splitedAge = fields[i].header.split("<=age<");
            filterParamsArray.push(["age", ">=".concat(splitedAge[0])]);
	    filterParamsArray.push(["age", "<".concat(splitedAge[1])]);
	    //break; //because I cannot deal with 30~40 50~60 at the same time currently
	}
	if (fields[i].tooltip.indexOf("Disease") !== -1) { //detect disease filter
	    console.error(i.toString().concat(" is the disease column, let's see what's inside"));
	    var diseaseName = fields[i].header.replace("_", " "); // this is the abcdefg code of the row
	    console.error(diseaseName);
	    filterParamsArray.push(["disease", diseaseName]);
	}
	if (fields[i].tooltip.indexOf("Tissue") !== -1) { //detect disease filter
            console.error(i.toString().concat(" is the tissue column, let's see what's inside"));
            var tissueName = fields[i].header.replace("_", " "); // this is the abcdefg code of the row
            console.error(tissueName);
            filterParamsArray.push(["tissue", tissueName]);
        }
        if (fields[i].tooltip.indexOf("Sexuality") !== -1) { //detect disease filter
            console.error(i.toString().concat(" is the sex column, let's see what's inside"));
            var sexName = fields[i].header.replace("_", " "); // this is the abcdefg code of the row
            console.error(sexName);
            filterParamsArray.push(["sex", sexName]);
        }
...
*/
function fromL3Variant(result) {
    var obj = jQuery.parseJSON(result.responseText);
    var table = variantTable;
    for (i = 0; i < obj.length; i++) {
	table += "<tr>";
        table += "<td>" + obj[i].chr + "</td>";
        table += "<td>" + obj[i].pos + "</td>";
        table += "<td>" + obj[i].ref + "</td>";
        table += "<td>" + obj[i].alt + "</td>";
        table += "<td>" + obj[i].homoCount + "</td>";
        table += "<td>" + obj[i].sampleCount + "</td>";
        table += "<td>" + obj[i].sampelCovered+ "</td>";
        table += "<td>" + obj[i].af+ "</td>";
        table += "<td>" + obj[i].annotation.consequence + "</td>";
        table += "<td>" + obj[i].annotation.aminoAcidChange + "</td>";
	table += "</tr>";       
    }
    table += "</table></body>";
    if (obj.length == 0) {
